...
def StringTreatment(df):
    ...
    df.loc[df.Region ==" Distints punts de Mallorca", 'Region'] = "Illes Balears"
...
def main ():
    ...
    InputExcel_FileName= str(year)+"Sona9Grups.csv"
    OutputExcel_FileName= str(year)+"Sona9GrupsWithLatLon.csv"    
    ...
    df.Poblacio.replace({'Casnet de Mar  (Maresme)':'Canet de Mar  (Maresme)'}, inplace = True)
    df.Poblacio.replace({'Distints punts de Mallorca (Pla de Mallorca)':'Mallorca'}, inplace = True)
    df.Poblacio.replace({'Mallorca-Lllubí (Pla de Mallorca)':'Mallorca'}, inplace = True)
    df.Poblacio.replace({'Santa Maria de Palutordera ':'Santa Maria de Palautordera '}, inplace = True)
